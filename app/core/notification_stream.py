...
async def append_notification_event(
    *,
    ...
    接口侧只做这一件事：XADD。真正写 notifications 表 + 推送接收者
    WebSocket 都在消费者里完成。
    """
...
async def _ensure_notification_group() -> None:
    ...
    try:
        await redis_client.xgroup_create(
            ...
            id=NOTIFICATION_GROUP_START_ID, mkstream=True,
        )
...
async def _claim_stale_pending(consumer_name: str) -> list[tuple[str, dict]]:
    ...
    pending = await redis_client.xpending_range(
        NOTIFICATION_STREAM, NOTIFICATION_GROUP, min="-", max="+",
        count=NOTIFICATION_BATCH_SIZE,
    )
    ids = [
        item["message_id"] for item in pending
        if int(item.get("time_since_delivered", 0)) >= NOTIFICATION_PENDING_IDLE_MS
    ]
    if not ids:
        return []
    return await redis_client.xclaim(
        NOTIFICATION_STREAM, NOTIFICATION_GROUP, consumer_name,
        NOTIFICATION_PENDING_IDLE_MS, ids,
    )
...
async def _read_messages(consumer_name: str) -> list[tuple[str, dict]]:
    ...
    try:
        streams = await redis_client.xreadgroup(
            NOTIFICATION_GROUP, consumer_name,
            {NOTIFICATION_STREAM: ">"},
            count=NOTIFICATION_BATCH_SIZE, block=NOTIFICATION_BLOCK_MS,
        )
...
async def _persist_notifications(db: AsyncSession, messages: list[tuple[str, dict]]) -> None:
    ...
    for message_id, fields in messages:
        try:
            ...
            logger.exception("通知落库失败，留 pending 重试: %s", message_id)
            await db.rollback()
            continue
        await redis_client.xack(NOTIFICATION_STREAM, NOTIFICATION_GROUP, message_id)
...
async def _notification_stream_loop() -> None:
    ...
    consumer_name = f"{socket.gethostname()}:notif:{id(asyncio.current_task())}"
...
async def start_notification_stream_task() -> None:
    ...
    if _notification_stream_task and not _notification_stream_task.done():
        return
    _notification_stream_task = asyncio.create_task(_notification_stream_loop())
